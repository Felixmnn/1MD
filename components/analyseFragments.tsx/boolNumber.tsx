...
type Props = {
  ...
  idA?: string;
  idB?: string;
  selectedField?: [string | null, number | null, number | null, number | null];
  isNumberBool?: boolean; // true = NumberBool, false = NumberEnum
};
 ...
 function generateNumberNumberStatements(r:number, slope:number, labelA:string, labelB:string, outliersA = 0, outliersB = 0) {
  ...
  const selectedText =
    selectedField && selectedField[0] !== null && selectedField[1] !== null
      ? t('boolNumber.selectedCase', {
          labelA,
          labelB,
          fieldA: selectedField[0],
          fieldB: selectedField[1],
          anteilA: selectedField[2] ?? '-',
          anteilB: selectedField[3] ?? '-',
        })
      : t('boolNumber.selectedNone');
